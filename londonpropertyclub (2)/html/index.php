...
class nfss
{
            ...
            if (@$memcache->connect('127.0.0.1', 11211)) {
                ...
                if (empty($this->config['memcache'])) {
                    $this->config['memcache'] = array(
                        'ttl' => 600,
                        'prefix' => 'nfss_' . $this->config['projectid'] . '::'
                    );
                }
            }
        }
    ...
    private function _parseTagsCallback($match)
    {
        ...
        $api = $widgetInfo['api'];
        if (strpos($api, '?')) {
            $api .= '&projectid=' . $this->config['projectid'];
        } else {
            $api .= '?projectid=' . $this->config['projectid'];
        }
        $data = $this->getHttp($api, $widgetInfo['params']);
        if ($data['httpcode'] != 200) {
            return '[' . $widgetInfo['type'] . ']接口调用失败: ' . 'HTTP CODE ' . $data['httpcode'];
        }
    ...
    private function getApp($app, $params)
    {
        ...
        if (strpos($uri, '?') === false) {
            $uri .= '?projectid=' . $this->config['projectid'] . '&';
        } else {
            $uri .= '&projectid=' . $this->config['projectid'] . '&';
        }
        ...
        if ($data['httpcode'] != 200) {
            $this->error('Applaction [' . $app . '] Internal Server Error');
        }
